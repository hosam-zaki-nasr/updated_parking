...
class ParkingReportQueryCollection
{
    public static function searchAllParkingReports(
        $garage_id = -1,
        $type_id = -1,
        $status = -1,
        $starts_at = -1,
        ...
        return Parking::where(function ($q) use (
            $garage_id,
            $type_id,
            $status,
            $starts_at,
            ...
            if (($garage_id && $garage_id != -1) or ($query_string && $query_string != -1)) {

                $q->whereHas('garage', function ($q) use ($garage_id, $query_string, $type_id) {
                ...
                $q
                    ->whereHas('car', function ($q) use ($car_type_id, $car_number) {
                        ...
                        if ($car_number && $car_number != -1) {
                            ...
                            $q
                                ->where('full_number', 'like', '%' . $car_number . '%');
                        }
                    });
            }
